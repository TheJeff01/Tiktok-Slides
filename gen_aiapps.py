...
ASSETS = f'{FILES}/ai-apps-2026-assets'
...
# ---------------- APP DATA ----------------
APPS = [
    dict(name='Cursor', desc='The AI-first code editor', cat='AI Code Editor', logo='app_cursor', light=True,
         blurb='A VS Code fork rebuilt around an AI that understands your <strong>entire</strong> codebase.',
         killer='Edit across multiple files from a single prompt.',
         best='Refactors &amp; features in large repos.',
         tip='It indexes your whole project — just ask in plain English and it knows where things live.'),
    dict(name='Claude', desc='by Anthropic', cat='AI Assistant', logo='app_claude', light=False,
         blurb="Anthropic's assistant — the one devs trust for hard reasoning, long code, and writing.",
         killer='Huge context — paste whole files and it keeps the thread.',
         best='Debugging, architecture &amp; code review.',
         tip='Its latest <strong>Opus-class</strong> models lead real-world coding work — and Claude Code runs them right in your terminal.'),
    dict(name='GitHub Copilot', desc='The original AI pair programmer', cat='AI Autocomplete', logo='app_githubcopilot', light=True,
         blurb='AI suggestions that live right inside the editor you already use.',
         killer='Ghost-text completions as you type, plus inline chat.',
         best='Staying in flow while you write code.',
         tip='Works in VS Code, JetBrains, Neovim and more — minimal setup, instant payoff.'),
    dict(name='Perplexity', desc='The answer engine', cat='AI Search', logo='app_perplexity', light=False,
         blurb='Search built for people who want answers <strong>with sources</strong> — not ten blue links.',
         killer='Real-time answers with links you can verify.',
         best='Researching errors, libraries &amp; best practices.',
         tip='Switch focus modes to search docs, GitHub, or the web only — and skip the SEO noise.'),
    dict(name='v0', desc='by Vercel', cat='AI UI Builder', logo='app_v0', light=True,
         blurb='Describe a UI in plain words, get production <strong>React + Tailwind</strong> back.',
         killer='Generates shadcn/ui components you can ship as-is.',
         best='Prototyping front-ends in minutes.',
         tip='Keep chatting to refine the design — no hand-editing CSS to get it right.'),
    dict(name='Warp', desc='The modern terminal', cat='AI Terminal', logo='app_warp', light=False,
         blurb='A terminal with AI that writes, explains, and fixes your commands.',
         killer='Ask in English — it gives you the exact command.',
         best='That one CLI flag you always forget.',
         tip='Blocks group each command with its output, so scrollback finally makes sense.'),
]
...
icons = (cover_icon('app_claude', 40, 150, 46, -8) + cover_icon('app_cursor', 32, 64, 50, 6) +
         cover_icon('app_perplexity', 96, 28, 42, -4) + cover_icon('app_v0', 120, 132, 44, 7) +
         cover_icon('app_warp', 176, 70, 46, -6) + cover_icon('app_githubcopilot', 92, 214, 40, 5))
      ...
      <div class="slide slide-dark">
        ...
        <div style="position:relative;z-index:3;display:flex;flex-direction:column;justify-content:flex-end;height:100%;padding:0 32px 50px;">
          ...
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:11px;">
            ...
            <span class="tag tag-dark">Developer Tools</span>
          </div>
          <div class="heading" style="color:#fff;font-size:32px;margin-bottom:13px;">AI Apps Every<br>Developer Should<br><span style="color:var(--green-light);">Be Using</span></div>
          <p class="body-text body-dark" style="max-width:262px;font-size:13px;">6 tools that quietly do half the job for you now. Swipe.</p>
        </div>
        {ARROW_D}
        {pbar(1)}
      </div>
      ...
      <div class="slide" style="background:var(--dark-bg);">
        ...
        <div style="position:relative;z-index:2;display:flex;flex-direction:column;justify-content:space-between;height:100%;padding:30px 32px 50px;">
          ...
          <div>
            ...
            <div class="heading" style="color:#fff;font-size:27px;margin-bottom:13px;line-height:1.15;">Your new<br><span style="color:var(--green-light);">unfair advantage.</span></div>
            <p class="body-text" style="font-size:13px;color:rgba(255,255,255,0.72);max-width:290px;margin-bottom:20px;">These won't replace you — but a developer who uses them will move twice as fast. Pick two and try them this week.</p>
            <div style="display:flex;gap:16px;margin-bottom:22px;">{stat('6 tools','this post')}{vdiv}{stat('AI','native')}{vdiv}{stat('Daily','dev content')}</div>
...
<title>Jeffthedev — AI Apps Every Developer Should Be Using (2026)</title>
...
{actions}
  ...
  <div class="ig-caption">
    <strong>Jeffthedev__</strong> 6 AI tools every developer should be using in 2026 — Cursor, Claude, GitHub Copilot, Perplexity, v0 and Warp. Save it, then pick two to try this week. Follow for daily developer content. #ai #webdev #developer #coding #cursor #softwaredevelopment #jeffthedev
  </div>
...
open(f'{FILES}/ai-apps-2026.html', 'w').write(doc)
